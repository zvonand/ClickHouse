...
MaterializedPostgreSQLConsumer::StorageData::StorageData(const StorageInfo & storage_info, LoggerPtr log_)
    ...
    , table_description(storage_info.storage->getInMemoryMetadataPtr(CurrentThread::get().tryGetQueryContext(), false)->getSampleBlock())
    , columns_attributes(storage_info.attributes)
    , column_names(storage_info.storage->getInMemoryMetadataPtr(CurrentThread::get().tryGetQueryContext(), false)->getColumns().getNamesOfPhysical())
    , array_info(createArrayInfos(storage_info.storage->getInMemoryMetadataPtr(CurrentThread::get().tryGetQueryContext(), false)->getColumns().getAllPhysical(), table_description))
{
