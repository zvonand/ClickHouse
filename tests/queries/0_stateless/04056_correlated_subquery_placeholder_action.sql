...
-- action nodes), it must skip in-place set building and produce a proper error
-- (correlated subqueries in IN are not yet supported).
...
-- Correlated subquery in IN clause: the subquery references t1.a from the outer query,
-- which produces PLACEHOLDER action nodes in the subquery plan.
-- This should produce a clear NOT_IMPLEMENTED error, not a PLACEHOLDER crash.
SELECT a, b
FROM t1
WHERE b IN (SELECT x FROM t2 WHERE t2.y = t1.a * 100)
ORDER BY a; -- { serverError NOT_IMPLEMENTED }
