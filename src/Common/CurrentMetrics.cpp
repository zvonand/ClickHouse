...
#define APPLY_FOR_BUILTIN_METRICS(M) \
    ...
    M(KeeperReadThreadsScheduled, "Number of queued or active jobs in the threadpool for keeper server reads. Meningless metric, the actual read tasks on this thread pool are scheduled through a different mechanism.") \
