...
ColumnPtr ColumnString::replicate(const Offsets & replicate_offsets) const
{
    ...
    /// This is a dependent prefix-sum: each iteration's accumulation depends on
    /// the previous one. On x86-64-v3, Clang auto-vectorizes to 256-bit (4x u64)
    /// with a horizontal reduction, which is slower than 128-bit (2x u64) because
    /// the wider SIMD doesn't improve throughput for dependent chains and just adds
    /// instruction overhead. Measured ~50% regression on the JOIN replication path.
