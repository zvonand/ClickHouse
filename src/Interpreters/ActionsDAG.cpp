        ...
        switch (node_type)
        {
            ...
            case ActionsDAG::ActionType::FUNCTION:
            {
                ...
                if (!function_overload_resolver)
                {
                    ...
                    /// Replacing equivalent `USING` columns can change child types in the rebuilt DAG.
                    /// Re-resolve the function with the actual arguments, otherwise cloned function metadata
                    /// may keep an outdated return type from the original filter DAG.
                    if (types_changed)
                    {
