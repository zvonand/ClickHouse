...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, parallel_read_threads, 8, "Number of threads for parallel local read request processing. 0 means disabled.", 0) \
...
void KeeperConfigurationAndSettings::dump(WriteBufferFromOwnString & buf) const
{
    ...
    for (const auto & field : coordination_settings.impl->all())
    {
