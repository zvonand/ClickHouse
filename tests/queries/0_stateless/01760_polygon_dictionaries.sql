...
PRIMARY KEY key
SOURCE(CLICKHOUSE(HOST 'localhost' PORT tcpPort() USER 'default' TABLE 'polygons' DB '{CLICKHOUSE_DATABASE_1}'))
LIFETIME(0)
LAYOUT(POLYGON())
...
SELECT tuple(x, y) as key,
    dictGet('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'name', key),
    dictGet('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'value', key),
    dictGet('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'value_nullable', key)
...
SELECT tuple(x, y) as key,
    dictGetOrDefault('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'name', key, 'DefaultName'),
    dictGetOrDefault('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'value', key, 30),
    dictGetOrDefault('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'value_nullable', key, 40)
FROM {CLICKHOUSE_DATABASE_1:Identifier}.points
ORDER BY x, y;
...
SELECT tuple(x, y) as key,
    dictHas('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', key),
    dictHas('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', key),
    dictHas('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', key)
FROM {CLICKHOUSE_DATABASE_1:Identifier}.points
ORDER BY x, y;
...
SELECT tuple(nan, inf) as key, dictGet('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'name', key); --{serverError BAD_ARGUMENTS}
SELECT tuple(nan, nan) as key, dictGet('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'name', key); --{serverError BAD_ARGUMENTS}
SELECT tuple(inf, nan) as key, dictGet('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'name', key); --{serverError BAD_ARGUMENTS}
SELECT tuple(inf, inf) as key, dictGet('{CLICKHOUSE_DATABASE_1:Identifier}.dict_array', 'name', key); --{serverError BAD_ARGUMENTS}
