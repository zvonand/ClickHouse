...
- `code` ([Int32](/sql-reference/data-types/int-uint)) — Error code.
- `error` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — Error name.
- `value` ([UInt64](/sql-reference/data-types/int-uint)) — Number of errors happened in time interval.
...
- `last_error_query_id` ([String](/sql-reference/data-types/string)) — Id of a query that caused the last error (if available).
