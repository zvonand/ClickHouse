...
#define DATABASE_ICEBERG_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, polaris_style_paths, false, "Enable Polaris/ADLS Gen2 path convention: the container name is prepended to the path in ABFSS locations (e.g. abfss://c@account/c/actual/path). When enabled, the redundant container prefix is stripped when building Azure HTTPS URLs.", 0) \
