...
Currently, `MinMax` and `NullCount` statistics support part pruning. When MinMax statistics are defined on a column, ClickHouse tracks the minimum and maximum values for that column in each part.
When NullCount statistics are defined on a `Nullable` column, ClickHouse tracks the number of NULL values in each part, enabling pruning based on `IS NULL` / `IS NOT NULL` predicates and improving range filter pruning accuracy for columns with NULL values.
