...
-- count() queries can use the exact count optimization which computes the result from primary key
-- analysis without reading data, so they bypass the max_rows_to_read limit.
...
-- Test fail-fast with multiple threads - count() bypasses via exact count, but SELECT * should still throw
SET max_threads = 4;
SET read_overflow_mode = 'throw';
