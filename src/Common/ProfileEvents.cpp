...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    \
    M(Shards, "The number of shards involved in a query, summed across all distributed tables and table functions. A single host is counted multiple times if it appears in multiple tables.", ValueType::Number) \
    \
