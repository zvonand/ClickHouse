...
/// ---------- primitive ----------
///
/// `DeleteBitmap` is a thin wrapper over CRoaring's `roaring::Roaring`.
/// One smoke test gates the wrapper layer (add / contains / cardinality /
/// empty / merge / addMany / toVector); the rest of the suite exercises
/// our own logic — `containsBulk`, `rangeCardinality` clamping,
/// serialize / deserialize format invariants, and filename helpers.
...
TEST(DeleteBitmapTest, WrapperSmoke)
{
    ...
    auto v = a.toVector();
    EXPECT_EQ(v, (std::vector<UInt32>{10, 20, 30, 40, 50}));
}
...
TEST(DeleteBitmapTest, ContainsBulkMatchesScalar)
{
    ...
    {
        DeleteBitmap empty;
        std::vector<UInt32> rows = {0, 1, 999, std::numeric_limits<UInt32>::max()};
    ...
    std::vector<UInt32> rows = {
        ...
        2'000'000, 4'000'000, std::numeric_limits<UInt32>::max()};
...
TEST(DeleteBitmapTest, RangeCardinalityRangeAboveUInt32MaxIsZero)
{
    /// Roaring32 only addresses UInt32 row IDs. A range entirely above
    /// `UInt32::max` cannot intersect the bitmap — the count must be 0
    /// even when row `UInt32::max` is set.
    DeleteBitmap bm;
    bm.add(std::numeric_limits<UInt32>::max());
...
TEST(DeleteBitmapTest, RoundtripSparse1M)
{
    ...
    {
        UInt32 v = samples[idx_dist(rng2)];
...
TEST(DeleteBitmapTest, VersionMismatchRejected)
{
    ...
    /// Bump the version field (bytes [4..8]) to 2.
    UInt32 bogus_version = 2;
...
TEST(DeleteBitmapTest, FileNameRoundtrip)
{
    ...
    EXPECT_EQ(DeleteBitmap::parseBlockNumberFromFileName("delete_bitmap_0.rbm"), std::optional<UInt64>(0));
    EXPECT_EQ(DeleteBitmap::parseBlockNumberFromFileName("delete_bitmap_999.rbm"), std::optional<UInt64>(999));
    EXPECT_EQ(DeleteBitmap::parseBlockNumberFromFileName("delete_bitmap_.rbm"), std::nullopt);
    EXPECT_EQ(DeleteBitmap::parseBlockNumberFromFileName("delete_bitmap_abc.rbm"), std::nullopt);
    EXPECT_EQ(DeleteBitmap::parseBlockNumberFromFileName("foo.rbm"), std::nullopt);
    EXPECT_EQ(DeleteBitmap::parseBlockNumberFromFileName("delete_bitmap_1"), std::nullopt);
    EXPECT_EQ(DeleteBitmap::parseBlockNumberFromFileName("delete_bitmap_1.rbm.tmp"), std::nullopt);
    EXPECT_EQ(DeleteBitmap::parseBlockNumberFromFileName(""), std::nullopt);
}
...
TEST(DeleteBitmapBench, DISABLED_ContainsBulkVsScalar)
{
    auto bench_one = [](const char * label, const DeleteBitmap & bm, UInt32 probe_max) {
        ...
        constexpr size_t PROBES = 10'000'000;
        std::vector<UInt32> probes(PROBES);
        for (auto & p : probes)
            p = dist(rng);
    ...
    /// Sparse 1M — random in [0, 1e8].
    {
        ...
        std::mt19937 rng(1);
        std::uniform_int_distribution<UInt32> d(0, 100'000'000);
        std::vector<UInt32> samples(1'000'000);
