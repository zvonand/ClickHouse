...
def test_iceberg_file_stats_logging(
    ...
    # Execute the query to trigger the log messages
    result = instance.query(f"SELECT * FROM {table_function_expr}")
    assert len(result.strip().split("\n")) == NUM_ROWS

    # Parse logged file stats
    stats = parse_logged_file_stats(instance)
    logging.info(f"Parsed file stats from logs: {stats}")
    assert len(stats) > 0, "Expected at least one file with logged stats"
    ...
    total_record_count = sum(s.get("record_count", 0) for s in stats.values())
...
@pytest.mark.parametrize("format_version", ["1", "2"])
@pytest.mark.parametrize("storage_type", ["s3"])
def test_iceberg_file_stats_logging_cluster(
    started_cluster_iceberg_with_spark, format_version, storage_type
):
    """
    Verify that record_count and file_size_in_bytes survive serialization
    through the cluster function protocol and are logged on worker nodes.
    """
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    spark = started_cluster_iceberg_with_spark.spark_session
    bucket = started_cluster_iceberg_with_spark.minio_bucket
    ...
    # Execute via cluster table function
    ...
    result = instance.query(f"SELECT * FROM {table_function_expr_cluster}")
    assert len(result.strip().split("\n")) == NUM_ROWS

    # Check that at least one node logged the file stats
    all_stats = {}
    ...
    assert len(all_stats) > 0, (
        "Expected at least one node to log Iceberg file stats via cluster function"
    )
    ...
    for file_path, file_stats in all_stats.items():
        ...
        # Verify record_count matches actual row count
        actual_row_count = int(
            instance.query(
                f"SELECT count() FROM s3('http://minio1:9001/{bucket}/{file_path}', 'minio', '{minio_secret_key}', 'Parquet')"
            ).strip()
