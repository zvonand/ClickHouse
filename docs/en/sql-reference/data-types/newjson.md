...
## Full-text search on JSON columns {#full-text-search-on-json}

[Text indexes](/engines/table-engines/mergetree-family/textindexes.md) can be used to accelerate full-text search on JSON columns.
The [`JSONAllValues`](../functions/json-functions.md#JSONAllValues) function returns all values from a JSON column as `Array(String)`, which can be indexed by a text index.
See [Indexing JSON columns](/engines/table-engines/mergetree-family/textindexes.md#text-index-example-json) in the text indexes documentation for details and examples.
