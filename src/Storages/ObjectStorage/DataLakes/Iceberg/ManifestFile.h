...
struct ParsedManifestFileEntry : boost::noncopyable
{
    ...
    /// The original file path string from Iceberg metadata (may be relative or absolute).
    ...
    ParsedManifestFileEntry(
        ...
        String file_path_from_metadata_,
        Int64 row_number_,
        ...
        , file_path_from_metadata(std::move(file_path_from_metadata_))
...
struct ProcessedManifestFileEntry
{
    ...
    /// Absolute file path
    /// It could have already been absolute in the manifest file
    /// Or resolved from parsed_entry->file_path_from_metadata + table_location
