...
EXPLAIN SYNTAX SELECT avg(log(2) * number) AS k FROM numbers(10000000) GROUP BY number % 5, ((number % 5) * (number % 5)) HAVING ((number % 5) * (number % 5)) < 5 ORDER BY k SETTINGS enable_analyzer=0, enable_optimize_predicate_expression=1; -- CI may inject False; HAVING condition on GROUP BY key not pushed to WHERE → stays as HAVING instead of WHERE
