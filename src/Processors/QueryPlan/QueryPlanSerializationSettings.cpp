...
#define PLAN_SERIALIZATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, enable_fixed_range_hash_table, true, R"(Enable converting the hash table to a flat array for joins when the key is a single integer with a small value range.)", 0) \
    DECLARE(UInt64, fixed_range_hash_table_max_size, 100'000, R"(Maximum range (max - min + 1) of join key values for the fixed range hash table optimization to apply.)", 0)
