...
- Path-relative URL (e.g. `data.csv`): merged with the base URL path. The last segment of the base path is replaced, so a trailing slash matters: `https://example.com/dir/` + `data.csv` = `https://example.com/dir/data.csv`, but `https://example.com/dir` + `data.csv` = `https://example.com/data.csv`.
...
- Query-only reference (e.g. `?x=1`): appended to the base URL path.
- Fragment-only reference (e.g. `#frag`): appended to the base URL (preserving any query string).
...
Dot segments (`../`, `./`) in relative URLs are preserved as-is and not normalized.
)", 0) \
