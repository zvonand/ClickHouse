...
{
    bool is_equal = true;
    checkImmutableFieldsEquals(from_zk, columns, virtual_columns, table_name_for_error_message, context, check_index_granularity);
...
StorageInMemoryMetadata ReplicatedMergeTreeTableMetadata::Diff::getNewMetadata(const ColumnsDescription & new_columns, const NamesAndTypesList & virtual_columns, ContextPtr context, const StorageInMemoryMetadata & old_metadata) const
{
    ...
    if (!empty())
    {
        ...
        if (sorting_key_changed)
        {
            ...
            new_metadata.sorting_key.recalculateWithNewAST(order_by_ast, new_metadata.columns, virtual_columns, context);

            if (new_metadata.primary_key.definition_ast == nullptr)
            {
                /// Primary and sorting key become independent after this ALTER so we have to
                /// save the old ORDER BY expression as the new primary key.
                new_metadata.primary_key = KeyDescription::getKeyFromAST(old_metadata.sorting_key.definition_ast, new_metadata.columns, virtual_columns, context);
            }
        }
    ...
    if (!sorting_key_changed) /// otherwise already updated
        new_metadata.sorting_key.recalculateWithNewColumns(new_metadata.columns, virtual_columns, context);

    /// Primary key is special, it exists even if not defined
    if (new_metadata.primary_key.definition_ast != nullptr)
    {
        new_metadata.primary_key.recalculateWithNewColumns(new_metadata.columns, virtual_columns, context);
    }
    else
    {
        new_metadata.primary_key = KeyDescription::getKeyFromAST(new_metadata.sorting_key.definition_ast, new_metadata.columns, virtual_columns, context);
