...
for _ in $(seq 1 60); do
    ...
    count=$($CLICKHOUSE_CLIENT --query "SELECT count() FROM system.query_log WHERE current_database = currentDatabase() AND event_date >= yesterday() AND event_time >= now() - 600 AND type = 'QueryFinish' AND query_id IN ('${CLICKHOUSE_DATABASE}_q1', '${CLICKHOUSE_DATABASE}_q2')")
    [ "$count" -ge 2 ] && break
    sleep 0.5
done
