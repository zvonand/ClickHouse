...
class ReleaseInfo:
    ...
    def prepare(
        ...
        if release_type == "patch":
            ...
            # Refuse to release a ref that is at-or-before the previously released
            # commit on this branch. cmake/autogenerated_versions.txt on the branch
            # tip records VERSION_GITHASH = the commit of the most recent release;
            # if `commit_ref` is an ancestor of that commit (or equal to it), tagging
            # would create an out-of-order release on a sibling/older commit.
            # Skipped on a brand-new branch (PATCH == 1, no prior release recorded).
            branch_cmake = Shell.get_output_or_raise(
                f"git show origin/{release_branch}:{FILE_WITH_VERSION_PATH}"
            )
            branch_patch_match = re.search(
                r"SET\(VERSION_PATCH\s+(\d+)\)", branch_cmake
            )
            assert (
                branch_patch_match
            ), f"VERSION_PATCH not found in cmake on origin/{release_branch}"
            if int(branch_patch_match.group(1)) != 1:
                branch_githash_match = re.search(
                    r"SET\(VERSION_GITHASH\s+([0-9a-f]+)\)", branch_cmake
                )
                assert (
                    branch_githash_match
                ), f"VERSION_GITHASH not found in cmake on origin/{release_branch}"
                prev_release_sha = branch_githash_match.group(1)
                if Shell.check(
                    f"git merge-base --is-ancestor {commit_ref} {prev_release_sha}"
                ):
                    raise RuntimeError(
                        f"Refusing to release ref [{commit_ref}]: it is at or "
                        f"before the previously released commit [{prev_release_sha}] "
                        f"on branch [{release_branch}]. Tagging would create an "
                        f"out-of-order release."
                    )
            if version.patch == 1:
