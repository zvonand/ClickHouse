...
private:
    ...
    bool insertIntoDynamicPath(ColumnDynamic & column_dynamic, const typename JSONParser::Element & element, const JSONExtractInsertSettings & insert_settings, const FormatSettings & format_settings, String & error) const
    {
        ...
        switch (element.type())
        {
            case ElementType::BOOL:
            {
                if (variant_info.variant_name_to_discriminator.contains("Bool") || column_dynamic.addNewVariant(getDataTypeCache().getType("Bool"), "Bool"))
                {
            ...
            case ElementType::INT64:
            {
                if (variant_info.variant_name_to_discriminator.contains("Int64") || column_dynamic.addNewVariant(getDataTypeCache().getType("Int64"), "Int64"))
                {
            ...
            case ElementType::UINT64:
            {
                if (variant_info.variant_name_to_discriminator.contains("UInt64") || column_dynamic.addNewVariant(getDataTypeCache().getType("UInt64"), "UInt64"))
                {
            ...
            case ElementType::DOUBLE:
            {
                if (variant_info.variant_name_to_discriminator.contains("Float64") || column_dynamic.addNewVariant(getDataTypeCache().getType("Float64"), "Float64"))
                {
            ...
            case ElementType::STRING:
            {
                ...
                {
                    if (variant_info.variant_name_to_discriminator.contains("String") || column_dynamic.addNewVariant(getDataTypeCache().getType("String"), "String"))
                    {
    ...
    bool insertIntoSharedData(WriteBuffer & buf, const typename JSONParser::Element & element, const JSONExtractInsertSettings & insert_settings, const FormatSettings & format_settings, String & error, MutableColumnPtr & tmp_dynamic_column) const
    {
        ...
        switch (element.type())
        {
            case ElementType::BOOL:
            {
                encodeDataType(getDataTypeCache().getType("Bool"), buf);
                writeBinary(element.getBool(), buf);
                return true;
            }
            case ElementType::INT64:
            {
                encodeDataType(getDataTypeCache().getType("Int64"), buf);
                writeBinaryLittleEndian(element.getInt64(), buf);
                return true;
            }
            case ElementType::UINT64:
            {
                encodeDataType(getDataTypeCache().getType("UInt64"), buf);
                writeBinaryLittleEndian(element.getUInt64(), buf);
                return true;
            }
            case ElementType::DOUBLE:
            {
                encodeDataType(getDataTypeCache().getType("Float64"), buf);
            ...
            case ElementType::STRING:
            {
                ...
                if (!format_settings.try_infer_dates && !format_settings.try_infer_datetimes)
                {
                    encodeDataType(getDataTypeCache().getType("String"), buf);
                ...
                if (format_settings.try_infer_dates)
                {
                    ...
                    if (tryInferDateFromString(data, date))
                    {
                        encodeDataType(getDataTypeCache().getType("Date"), buf);
                ...
                if (format_settings.try_infer_datetimes && !format_settings.try_infer_datetimes_only_datetime64)
                {
                    ...
                    if (tryInferDateTimeFromString(data, value, format_settings, time_zone_for_schema_inference, utc_time_zone_for_schema_inference))
                    {
                        encodeDataType(getDataTypeCache().getType("DateTime"), buf);
...

                {
                    ...
                    if (tryInferDateTime64FromString(data, value, format_settings, time_zone_for_schema_inference, utc_time_zone_for_schema_inference))
                    {
                        encodeDataType(getDataTypeCache().getType("DateTime64(9)"), buf);
