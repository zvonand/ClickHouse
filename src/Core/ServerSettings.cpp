...
The policy on how to perform a scheduling of CPU slots specified by `concurrent_threads_soft_limit_num` and `concurrent_threads_soft_limit_ratio_to_cores`. Algorithm used to govern how limited number of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.
    ...
    **Example**
    ...
    DECLARE(UInt64, predicate_statistics_sample_rate, 0, R"(Collect predicate selectivity statistics and write them to `system.predicate_statistics_log`. Filter-level statistics are sampled every N chunks per `FilterTransform` instance. Index-level statistics are logged once per MergeTree read when enabled. 0 means disabled, 1 means every chunk/query.)", 0) \
