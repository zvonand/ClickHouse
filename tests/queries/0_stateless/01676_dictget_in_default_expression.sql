...
CREATE DICTIONARY {CLICKHOUSE_DATABASE_1:Identifier}.dict (key UInt64, value UInt64) PRIMARY KEY key SOURCE(CLICKHOUSE(DB '{CLICKHOUSE_DATABASE_1}' TABLE 'dict_data' HOST '127.0.0.1' PORT tcpPort())) LIFETIME(0) LAYOUT(HASHED());

CREATE TABLE {CLICKHOUSE_DATABASE_1:Identifier}.table (x UInt64, y UInt64 DEFAULT dictGet('{CLICKHOUSE_DATABASE_1:Identifier}.dict', 'value', x)) ENGINE=MergeTree ORDER BY tuple();
...
SELECT 'status:';
SELECT status FROM system.dictionaries WHERE database='{CLICKHOUSE_DATABASE_1}' AND name='dict';
...
SELECT COALESCE((SELECT status FROM system.dictionaries WHERE database='{CLICKHOUSE_DATABASE_1}' AND name='dict')::Nullable(String), 'NOT_LOADED');
