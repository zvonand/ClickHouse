...
The autogenerator (`tests/ci/changelog.py`) converts every PR description in
the release range into a bullet under the category the author picked. The
maintainer then heavily edits that output before it lands in `CHANGELOG.md`.
This skill applies those edits.
...
- `$0` (required): path to the auto-generated changelog file (the output of
  `utils/changelog/changelog.py --output=...`). Edit this file in place.

If the user instead points you at a section of `CHANGELOG.md` that's already
been pasted in, treat that section as the input and edit it in place.
...
releases. Frequencies are approximate; for exact examples consult the
companion file `examples.md` in this skill directory.
...
### 2. Drop entire `#### NO CL ENTRY` section

These are pure-revert PRs (`Revert "..."`) that the autogenerator includes
because the revert PR has no `Changelog entry`. They never appear in the
final `CHANGELOG.md`. Delete the section header and all its bullets.
...
- If the entry mentions a user-visible behaviour (function/setting name, a
  ...
  the rules in §6 to pick the category). Strip developer-internal
  preambles ("This closes:", a bare issue link with no description, "fix
  msan ...", "ci: ..."). If after stripping there is no real user-facing
  description, drop the entry instead of promoting an empty one.
- Otherwise — delete it.
...
- Trailing `Closes: https://...` / `Fixes: https://github.com/ClickHouse/...`
  lines that duplicate the issue closure — cut them. (Inline `Closes #N` /
  `Fixes #N` *inside* the description is fine and gets auto-linked.)
...
- **`Fix`/`Fixed`/`Fixes`-shaped entry → Bug Fix** unless it's explicitly
  fixing a perf regression. The author's chosen category is often
  `Improvement` for entries that are really bug fixes.
...
The preferred category order (from `tests/ci/changelog.py`) is:
...
Detect entries that cover the same subject and merge them. Strong
indicators:

- Same library/component name (`chdig`, `Iceberg`, `DeltaLake`, text-index,
  query-tree, `play.html`, base58, etc.).
- One PR is explicitly a follow-up of another (`Follow-up to #N`,
  `continuation of #N`).
- One PR enables/promotes what the other added (e.g. new feature + GA-promotion).
