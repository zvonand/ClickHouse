...
- `kafka_autodetect_client_rack` — Use cloud facilities to set `client.rack` librdkafka parameter to communicate with the nearest kafka replica. Supported providers: `MSK` (with alias `AWS/IMDSv2/availability-zone-id`), `CONFLUENT` (with alias `AWS/IMDSv2/availability-zone`), `GCP` (with alias `GCP/MDS/zone`) and `CLICKHOUSE`. `CLICKHOUSE` means internal facilities that are controlled by dedicated configuration parameters. Note that alias is just a string, it is not parsed. Disabled by default.
