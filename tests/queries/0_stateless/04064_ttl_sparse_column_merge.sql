...
-- TTL is added via ALTER after data is already inserted, so that existing parts
-- have no TTL info and the merge runs with force_ttl = true.
-- With ts = 0, `isTTLExpired` returns false (by design), so no rows are deleted.
...
-- Add TTL after inserting data: existing parts lack TTL info,
-- so the merge recalculates TTL with force_ttl = true, exercising
-- the code path where the sparse column is returned from the block directly.
ALTER TABLE t_ttl_sparse MODIFY TTL ts;
