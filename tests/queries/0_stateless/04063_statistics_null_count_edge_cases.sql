...
-- =============================================================================
-- Test 3: optimize_functions_to_subcolumns = 0
-- =============================================================================
DROP TABLE IF EXISTS test_nullcount_subcol0;

SET optimize_functions_to_subcolumns = 0;

CREATE TABLE test_nullcount_subcol0 (
    id UInt64,
...
-- Part 0: all NULL
INSERT INTO test_nullcount_subcol0 SELECT 0, NULL;
-- Part 1: no NULL
INSERT INTO test_nullcount_subcol0 SELECT 1, 100;
-- Part 2: mixed
INSERT INTO test_nullcount_subcol0 SELECT 2 + number, if(number % 2 = 0, NULL, number + 200) FROM numbers(100);

ALTER TABLE test_nullcount_subcol0 MATERIALIZE STATISTICS value SETTINGS mutations_sync = 1;

SET use_statistics_for_part_pruning = 1;

SELECT 'Test 3a: IS NULL with subcolumns=0 (Parts 1 pruned)';
SELECT trimLeft(explain) FROM (EXPLAIN indexes = 1 SELECT count() FROM test_nullcount_subcol0 WHERE value IS NULL)
WHERE explain LIKE '%Parts:%' OR explain LIKE '%Ranges:%';
SELECT count() FROM test_nullcount_subcol0 WHERE value IS NULL;

SELECT 'Test 3b: IS NOT NULL with subcolumns=0 (Part 0 pruned)';
SELECT trimLeft(explain) FROM (EXPLAIN indexes = 1 SELECT count() FROM test_nullcount_subcol0 WHERE value IS NOT NULL)
WHERE explain LIKE '%Parts:%' OR explain LIKE '%Ranges:%';
SELECT count() FROM test_nullcount_subcol0 WHERE value IS NOT NULL;

SET optimize_functions_to_subcolumns = 1;

DROP TABLE test_nullcount_subcol0;
