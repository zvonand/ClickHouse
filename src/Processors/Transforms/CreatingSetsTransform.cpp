...
void CreatingSetsTransform::startSubquery()
{
    ...
    if (set_and_key->external_table)
        /// TODO: make via port
        table_out = QueryPipeline(set_and_key->external_table->write({}, set_and_key->external_table->getInMemoryMetadataPtr(CurrentThread::get().tryGetQueryContext(), false), nullptr, /*async_insert=*/false));
