...
CREATE TABLE test_stats_exceeds (
    ...
    val_float64 Float64              -- Float64: no 2^53 guard needed, large values are valid
) ENGINE = MergeTree()
...
-- =============================================================================
-- Case 8: Float64 with min < 2^53 and max > 2^53
-- Statistics: min=1000, max=1e17
-- For Float64, the ±2^53 integer-exactness guard does not apply because
-- Float64 values are stored losslessly. Range=[1000, 1e17], pruning works.
-- =============================================================================
...
SELECT '-- Case 8.1: Float64 min < 2^53, max > 2^53, query < min, should prune part 8';
...
SELECT '-- Case 8.2: Float64 min < 2^53, max > 2^53, query > max (2e17), should prune part 8';
...
-- =============================================================================
-- Case 9: Float64 with min < -2^53 and max > -2^53
-- Statistics: min=-1e17, max=-1000
-- Same as Case 8 but with negative values. Range=[-1e17, -1000], pruning works.
-- =============================================================================
...
SELECT '-- Case 9.1: Float64 min < -2^53, max > -2^53, query > max, should prune part 9';
...
SELECT '-- Case 9.2: Float64 min < -2^53, max > -2^53, query < min (-2e17), should prune part 9';
