...
TableJoin::TableJoin(const Settings & settings, VolumePtr tmp_volume_, TemporaryDataOnDiskScopePtr tmp_data_)
    ...
    , fixed_range_hash_table_max_size(std::min<UInt64>(settings[Setting::fixed_range_hash_table_max_size], 3'000'000)) /// Prevent FixedRangeHashTable from allocating a lot of memory.
...
TableJoin::TableJoin(const JoinSettings & settings, bool join_use_nulls_, VolumePtr tmp_volume_, TemporaryDataOnDiskScopePtr tmp_data_)
    ...
    , fixed_range_hash_table_max_size(std::min<UInt64>(settings.fixed_range_hash_table_max_size, 3'000'000)) /// Prevent FixedRangeHashTable from allocating a lot of memory.
