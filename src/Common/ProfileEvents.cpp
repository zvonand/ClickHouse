...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    \
    M(Shards, "The total expected number of shards involved in a query, which includes skipped shards with the `skip_unavailable_shards` setting, summed across all distributed tables and table functions. A single host is counted multiple times if it appears in multiple tables.", ValueType::Number) \
    \
