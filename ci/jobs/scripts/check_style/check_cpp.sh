...
join -v1 <(grep '\.h$' "$STYLE_TMPDIR/nobase_all" | sed 's:.*/::'  | sort -u) <(find $ROOT_PATH/{src,programs,utils,tests/lexer} -name '*.cpp' -or -name '*.c' -or -name '*.h' -or -name '*.S' | xargs rg --no-filename -o '[\w-]+\.h' | sort -u) |
...
xargs < "$STYLE_TMPDIR/nobase_excluded" grep -P '#include[\s]*(").*(")' "${QUOTE_EXCLUSIONS[@]}" | \
